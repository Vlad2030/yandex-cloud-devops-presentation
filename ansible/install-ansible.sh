...
if ! command -v ansible-playbook &>/dev/null; then
    if [ -f /etc/centos-release ] || [ -f /etc/redhat-release ] || [ -f /etc/oracle-release ] || [ -f /etc/system-release ] || grep -q 'Amazon Linux' /etc/system-release; then
        ...
        # Install required Python libraries and pip
        ...
        if [ -z "$(command -v pip)" ] && [ -z "$(command -v easy_install)" ]; then
            yum -y install python-setuptools
            easy_install pip
        elif [ -z "$(command -v pip)" ] && [ -n "$(command -v easy_install)" ]; then
            easy_install pip
        fi

        # Install passlib for encryption
        ...
        echo 'WARN: Could not detect distro or distro is unsupported'
